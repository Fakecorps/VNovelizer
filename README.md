...
![alt text](https://img.shields.io/badge/Unity-2022.3%2B-blue?logo=unity)
![alt text](https://img.shields.io/badge/License-MIT-green)
![alt text](https://img.shields.io/badge/Status-Active-brightgreen)
轻量级 · 高性能 · 数据驱动 · 零代码工作流
📖 使用文档 (飞书) | 📺 作者 B 站 | 🐛 提交 Issue
📖 简介 (Introduction)
...
在 Resources/VNovelizerRes/Characters 目录下（或自定义目录）。
右键点击 -> Create -> VNovelizer -> CharacterProfile。
配置面板说明：
...
第二步：编写剧本 (Scripting)
打开顶部菜单 VNovelizer -> 📜 剧本管理器。
点击 "➕ 新建"，输入文件名（如 Chapter1）。Excel 将自动打开。
剧本字段详解：
...
编辑完成后保存 Excel。
在剧本管理器中点击 "🔄 转换"，生成游戏所需的 .asset 数据文件。
第三步：运行游戏 (Run)
调试模式：
打开 Assets/Scenes/VNDebugScene 场景。
在 Inspector 面板输入剧本名（如 Chapter1）和起始行 ID。
点击运行，即可直接跳转测试。
代码调用：
在任意脚本中调用以下代码即可启动流程：
code
...
🎵 Music: D-wheat Music (moonlight chill, piano lofi)
🖼️ UI Assets: One Level Studio
📦 Icons: Prinbles
🏙️ Backgrounds: Noraneko Games
🎨 Frames: K-ramstack
📜 许可证 (License)
