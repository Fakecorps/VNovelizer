...
- **Assembly definitions**: The project has five assemblies. `VNovelizer.Runtime` (runtime core, references PrimeTween.Runtime + LitJson + Unity.InputSystem + TextMeshPro + Localization + Coffee.UIParticle + Alchemy). `VNovelizer.Editor` (editor-only tools, references Runtime + ExcelDataReader + Unity.Localization.Editor + Alchemy.Editor). `LitJson` (third-party JSON, no references). `Alchemy` (bundled Inspector attribute definitions, references Unity built-in modules). `Alchemy.Editor` (bundled Inspector/Hierarchy/EditorWindow drawing, editor-only, references Alchemy).

- **Dev test project**: The dedicated Unity project for testing and adjusting the plugin is located at `D:\Unity\Unity项目\VNovelizer_Dev1` (NOT `Vnovelizer_Dev` — that's an older/separate project). It references VNovelizer as a local file dependency (`"com.fakecorps.vnovelizer": "file:D:/VNovelizer"` in its `Packages/manifest.json`). Live2D SDK is imported directly into `Assets/Live2D/` (not via Package Manager), and the test model lives in `Assets/miku/`. After modifying prefabs, fonts, or other resources in the Dev1 project, copy the modified files back to `Runtime/PackageDefault/` in the package to sync changes. When syncing prefabs, only copy the `.prefab` file itself, NOT the `.prefab.meta` (to preserve the package's own GUIDs).
